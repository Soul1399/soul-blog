...
import {
  defineEmptyCell,
  getCellContextValues,
  StaticGridCellContext,
} from '../../models/ui/static-grid-cell-context';
...
export class StaticGridComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  headTitle: string = '';
  @Input()
  @OnChange<StaticGridConfiguration>(function (
    this: StaticGridComponent,
    conf,
    change
  ) {
  ...
  @Input()
  headTemplate: StaticGridTemplate = {
    outlet: null,
    outletContext: null,
    type: 'header',
  };
  @Input()
  footerTemplate: StaticGridTemplate = {
    outlet: null,
    outletContext: null,
    type: 'footer',
  };
  @Input()
  @OnChange<StaticGridHeader[] | null>(function (
    this: StaticGridComponent,
    headers,
    change
  ) {
    if (
      _.isArray(change.currentValue) &&
      _.isArray(change.previousValue) &&
      change.currentValue.length == change.previousValue.length &&
      _.intersection(change.currentValue, change.previousValue).length ==
        change.currentValue.length
    ) {
      return;
    }
  ...
  @Input()
  cellsOfRow?: ((row: StaticGridRowContext) => StaticGridCellOfRow[]) | null =
    null;
  @Input()
  rowData: any[] | null = [];
  @Output()
  ...
  ngOnInit(): void {
    this.mouseupSubscribe = fromEvent(document, 'mouseup').subscribe(
      (x: Event) => {
        ...
        if (this.rowsAreaRef && this.rowsAreaRef.nativeElement) {
          if (
            target != this.rowsAreaRef.nativeElement &&
            target.closest(`.${this.rowsAreaRef.nativeElement.classList[0]}`) ==
              null
          ) {
        ...
        if (editingCell && editingCell.elementId) {
          ...
          if (
            target != cell &&
            target.closest(`#${editingCell.elementId}`) == null
          ) {
    ...
    this.currentEditingCellSubscribe = this.currentEditingCell$.subscribe(
      (c) => {
        if (c != null) {
          c.isEditing = true;
          setTimeout(() => {
            const e = document.querySelector(
              `#${c.settings.cellType}_${c.elementId}`
            );
  ...
  buildRows(array: any) {
    ...
    if (this.configuration.cssRenderMode == 'grid') {
      let rowWithMaxCells = rows[0];
      rows.forEach((r) => {
        if (r.cells.length > rowWithMaxCells.cells.length) rowWithMaxCells = r;
      });
      this.cssGridTemplateColumns = rowWithMaxCells.cells.map((c) => `fit-content(${c.width})`).join(' ');
      // unapply row gap to all except last
      _.initial(rows).forEach((r) => {
        r.spaceAfter = '';
      });
    } else {
      this.cssGridTemplateColumns = '';
      // apply row gap to all except last
      _.initial(rows).forEach((r) => {
        r.spaceAfter = `${config.rowGap}px`;
      });
    }
  ...
  buildRow(x: any, position: number) {
    ...
    if (
      ...
      if (_.isFunction(config.commands.setRowHeight.command)) {
        config.commands.setRowHeight
          .execute(config, row, null, x, this.rowData)
          .pipe(first())
          .subscribe((h) => (row.height = h ?? ''));
      }
    }
  ...
  buildCells(row: StaticGridRowContext) {
    ...
    if (this.configuration.cssRenderMode == 'grid') {
      // unapply cell gap to all except last
      ...
      cells.forEach((c) => {
        ...
        c.cssStyle = Object.assign(c.cssStyle ?? {}, {
          width: '',
          'margin-right': '',
  ...
  buildCellsFromHeaders(
    ...
    if (
      _.isFunction(this.cellsOfRow) &&
      (cellsOfRow = this.cellsOfRow(row))?.length > 0
    ) {
      ...
      for (const header of this.headers) {
        if (_.isArray(header.relatedCells) && header.relatedCells.length > 0) {
          const columnsWidth = header.width
            ? Math.round(header.width / header.relatedCells.length)
            : null;
          for (const cellOfRow of header.relatedCells) {
            order++;
            const cell = this.createCellContext(
              row,
              cellOfRow,
              columnsWidth,
              order,
  ...
  createCellContext(
    row: StaticGridRowContext,
    cellOfRow: StaticGridCellOfRow,
    columnsWidth: number | null,
    order: number,
    header?: StaticGridHeader
  ): StaticGridCellContext {
    ...
    const cell: StaticGridCellContext = {
      ...
      width: `${
        cellOfRow.width || columnsWidth || this.configuration.cellWidth
      }px`,
      spaceAfter: '0',
      enabled:
        (header?.enabled ?? true) &&
      ...
      isEditing: false,
      isEditable:
        !!cellOfRow.cellType && cellOfRow.cellType != StaticGridCellType.label,
      cssClass: _.isFunction(cellOfRow.cssClass)
        ? cellOfRow.cssClass(row)
        : cellOfRow.cssClass?.slice() ?? [],
      cssStyle:
        typeof cellOfRow.placeItem === 'string'
          ? { display: 'grid', 'place-items': cellOfRow.placeItem }
          : {},
    ...
    const relatedValues = _.isFunction(cellOfRow.relatedValues)
      ...
      : [];
    cell.relatedValues =
      relatedValues instanceof Observable ? relatedValues : of(relatedValues);
  ...
  validateDirectEdit(
    ...
    if (cell.settings.cellType == StaticGridCellType.input) {
      this.setValueFromInput(row, cell, newValue)
        .pipe(first())
        .subscribe((c) => this.validateCell(cell));
    } else if (cell.settings.cellType == StaticGridCellType.select) {
      const item = newValue as SelectItem;
      this.setValueFromInput(row, cell, item.code)
        .pipe(first())
        .subscribe((c) => this.validateCell(cell));
  ...
  setValueFromInput(
    ...
    cell: StaticGridCellContext,
    inputValue: any
  ): Observable<StaticGridCellContext> {
    if (cell.settings.valueField) {
      const copy = _.clone(cell.row.data);
      copy[cell.settings.valueField] = inputValue;
      return this.configuration.commands.set
        .execute(this.configuration, row, cell, copy, this.rowData)
        .pipe(
          first(),
          map((result) => {
              ...
              if (this.configuration.forceRedrawOnChange) {
                row.cells = row.cells.map((c) => {
                  ...
                  if (c === cell) {
                    Object.assign(
                      cellCopy,
                      getCellContextValues(row, cell.settings)
                    );
                    defineEmptyCell(cellCopy);
                    this.changes.push({
                      key: cell.elementId ?? `${row.position}_${cell.order}`,
                      type: 'modify',
                      valueBefore: cell,
                      valueAfter: cellCopy,
                    });
                ...
                const change: StaticGridChange = {
                  key: cell.elementId ?? `${row.position}_${cell.order}`,
                  type: 'modify',
                  valueBefore: _.cloneDeep(cell),
                  valueAfter: {},
                };
  ...
  validateCell(
    ...
    if (!cell.isDirectEdit) {
      if (cell.settings.cellType == StaticGridCellType.input) {
        return this.setValueFromInput(cell.row, cell, newValue).pipe(
          first(),
          map((c) => !(c.isEditing = false))
        );
      } else if (cell.settings.cellType == StaticGridCellType.select) {
        const item = newValue as SelectItem;
        return this.setValueFromInput(cell.row, cell, item.code).pipe(
          first(),
          map((c) => !(c.isEditing = false))
        );
      }
    ...
    if (_.isArray(this.headers)) {
      this.headers.forEach((hd) => {
        if (_.isArray(hd.relatedCells) && hd.relatedCells.length > 0) {
          const totalWidth = hd.relatedCells.reduce(
            (x, y) => x + (y.width ?? 0),
            0
          );
